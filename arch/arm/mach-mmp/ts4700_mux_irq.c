...
static void mux_irq_handler(unsigned int irq, struct irq_desc *desc)
{
 ...
 //  while((fpga_irq_reg = FPGA_REG(FPGA_IRQ_OFFSET))) {
...
void __init ts4700_init_mux_irq(void)
{
   ...
   /*
    * On the TS-4700, the FPGA interrupt is connected to MFP_27.  This pin must
    * be configured as the SMC_IRQ pin (this is done in ts4700.c -- look for "GPIO27_GPIO").
    */
   ...
   /* Install mux handler */
   set_irq_chained_handler(IRQ_PXA168_SM_INT, mux_irq_handler);
}
