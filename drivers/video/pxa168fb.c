...
{
   struct fb_videomode *myVideoMode;
	struct pxa168fb_mach_info *mi;
   ...
   unsigned int baseboard;
	...
	printk(KERN_ERR "Kris: starting fb probe\n");
...
#else
	myVideoMode = &mi->modes[0];
#endif
#ifdef TS_4700_Base_LCD3_5_Test_P1
	myVideoMode = &mi->modes[7];
#endif

	fbi->dft_vmode.xres = myVideoMode->xres;
	fbi->dft_vmode.yres = myVideoMode->yres;
	fbi->dft_vmode.refresh = myVideoMode->refresh;
	 ...
	 */
	set_mode(fbi, &info->var, myVideoMode, mi->pix_fmt, 1);
...
static int _pxa168fb_suspend(struct pxa168fb_info *fbi, pm_message_t mesg)
{
	...
	if (cntrl0 & CFG_DMA_ENA_MASK) {
		writel(cntrl0 & ~CFG_DMA_ENA_MASK, fbi->reg_base + LCD_SPU_DMA_CTRL0);
	}
